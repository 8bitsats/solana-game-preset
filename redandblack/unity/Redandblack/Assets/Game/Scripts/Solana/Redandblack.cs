...
using Redandblack;
using Redandblack.Program;
using Redandblack.Errors;
using Redandblack.Accounts;
...
namespace Redandblack
{
    ...
    namespace Errors
    {
        public enum RedandblackErrorKind : uint
        {
    ...
    public partial class RedandblackClient : TransactionalBaseClient<RedandblackErrorKind>
    {
        public RedandblackClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.RedandblackProgram.InitPlayer(accounts, levelSeed, programId);
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.RedandblackProgram.ChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.RedandblackProgram.SuperChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<RedandblackErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<RedandblackErrorKind>>{{6000U, new ProgramError<RedandblackErrorKind>(RedandblackErrorKind.NotEnoughEnergy, "Not enough energy")}, {6001U, new ProgramError<RedandblackErrorKind>(RedandblackErrorKind.WrongAuthority, "Wrong Authority")}, };
        }
    }
    ...
    namespace Program
    {
        ...
        public static class RedandblackProgram
        {
